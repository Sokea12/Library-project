...
@section('content-header')
     <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">All Books</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="{{ route('books.create') }}">Add New Books</a></li>
...
<div class="card mb-4">
    ...
    <table class="table">
        ...
        <tr class="align-middle">
            ...
            <td>{{$book->authers->name ?? 'N/A'}}</td>
            <td>{{$book->publishers->name ?? 'N/A'}}</td>
            <td>
                @if ($book->status == 'Y')
            ...
            <td>
                <button class="btn btn-danger delete-book" data-toggle="modal" data-target="#exampleModal">
                    ...
                    <input type="hidden" name="books_name" id="books_name" value="{{ $book->name }}">
                    Delete
                </button>
            </td>
        </tr>
...
<script>
    $(document).ready(function() {
        $('.delete-book').on('click', function() {
            var bookId = $(this).find('input[name="books_id"]').val();
            var bookName = $(this).find('input[name="books_name"]').val();

            $('#book-name').text(bookName);

            // Base route with placeholder
            var baseRoute = "{{ route('books.destroy', ':id') }}";
            var finalRoute = baseRoute.replace(':id', bookId);
